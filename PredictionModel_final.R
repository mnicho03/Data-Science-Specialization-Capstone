...
#final model / data product does not require train/test split

final_split <- function(file) {
...
final_name <- paste(deparse(substitute(file)), "final", sep = "_")

#assign variables outside of the function
assign(final_name, final, envir = .GlobalEnv)
}

#run the functions and create all train/test datasets
final_split(mini_blogs)
final_split(mini_news)
final_split(mini_twitter)

#merge newly created files to create complete train & test sets: no need to continue treating the data by file
final <- rbind.data.frame(mini_blogs_final, mini_news_final, mini_twitter_final)
...
#training
file_connection <- file("full_corpus/final/final_corpus.txt")
writeLines(final, file_connection)
close(file_connection)

#set location for corpus file (as referenced above)
path_to_final <- file.path("full_corpus/final")

#preprocessing
#load in the corpus
final_corpus <- Corpus(DirSource(path_to_final))
...
final_corpus_cleaned <- suppressWarnings(corpus_cleaner(final_corpus))

#*in the current process, no stemming is conducted

#clear out environment other than the corpus
rm(list = setdiff(ls(), c("final_corpus_cleaned")))

#save off only the text portion of the corpus 
final_text <- c(final_corpus_cleaned[[1]][[1]])
#reduce RAM load
rm(final_corpus_cleaned)

#ngram building

#create df to use with dplyr manipulation below
final_text_df <- data.frame(text = final_text, stringsAsFactors = FALSE)
...
system.time(unigram_df <- unigram_df_build(final_text_df))
# #using 10% sample
# user  system elapsed 
# 14.64    0.21   14.93
...
system.time(bigram_df <- bigram_df_build(final_text_df))
#using 10% sample
# user  system elapsed 
# 66.01    1.06   72.84 
...
system.time(trigram_df <- trigram_df_build(final_text_df))
#using 10% sample
# user  system elapsed 
# 124.97    2.04  142.04 
...
system.time(quadgram_df <- quadgram_df_build(final_text_df))
#using 10% sample
# user  system elapsed 
# 155.28    2.27  170.38 
...
userInput_cleaner <- function(input){
        ...
        # 
        # #save globally
        # assign("userInput_words", userInput_words, envir = .GlobalEnv)
        # assign("userInput_words_length", userInput_words_length, envir = .GlobalEnv)
}
...
next_word_Prediction <- function(ngram_input, target_df) {
        ...
        #print the most likely results and the prediction
